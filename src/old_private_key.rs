...
use crate::msg::Msg;
use crate::old_public_key::OldPublicKey;
...
pub struct OldPrivateKey([u8; 32]);
...
impl OldPrivateKey {
    ...
    pub fn from_secret(secret: &[u8]) -> OldPrivateKey {
    ...
    pub fn from_phrase(phrase: &str, passphrase: &str) -> Result<OldPrivateKey, PrivateKeyError> {
        if phrase.is_empty() {
            return Err(HdWalletError::Bip39Error(Bip39Error::BadWordCount(0)).into());
        }
        OldPrivateKey::from_hd_wallet_path("m/44'/118'/0'/0/0", phrase, passphrase)
    }
    ...
    pub fn from_hd_wallet_path(
        ...
        phrase: &str,
        passphrase: &str,
    ) -> Result<OldPrivateKey, PrivateKeyError> {
    ...
    ) -> Result<OldTxParts, PrivateKeyError> {
        // prefix does not matter in this case, you could use a blank string
        let our_pubkey = self.to_public_key(OldPublicKey::DEFAULT_PREFIX)?;
        // Create TxBody
        let body = TxBody {
...
impl FromStr for OldPrivateKey {
...
#[test]
fn test_secret() {
    let private_key = OldPrivateKey::from_secret(b"mySecret");
    assert_eq!(
        private_key.0,
        [
...
fn test_cosmos_key_derivation_manual() {
    ...
    let private_key = OldPrivateKey(m44h_118h_0h_0_0);
...
fn test_cosmos_key_derivation_with_path_parsing() {
    ...
    let private_key = OldPrivateKey::from_phrase(words, "").unwrap();
    let public_key = private_key.to_public_key("cosmospub").unwrap();
    let address = public_key.to_address();
    assert_eq!(
        address.to_bech32("cosmos").unwrap(),
...
fn test_many_key_generation() {
    ...
    for _ in 0..1000 {
        ...
        let cosmos_key = OldPrivateKey::from_secret(&secret);
...
fn test_bad_phrase() {
    let cosmos_key = OldPrivateKey::from_phrase("bad phrase", "");
