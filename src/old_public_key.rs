...
impl OldPublicKey {
    ...
    pub fn from_slice<T: Into<String>>(bytes: &[u8], prefix: T) -> Result<Self, PublicKeyError> {
        ...
        OldPublicKey::from_bytes(result, prefix)
    }
    ...
    pub fn from_bytes<T: Into<String>>(
        bytes: [u8; 33],
        prefix: T,
    ) -> Result<OldPublicKey, PublicKeyError> {
        Ok(OldPublicKey {
            bytes,
    ...
    pub fn from_bech32(s: String) -> Result<OldPublicKey, PublicKeyError> {
        ...
        OldPublicKey::from_bytes(key, hrp)
    }
}

impl FromStr for OldPublicKey {
    type Err = PublicKeyError;
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if let Ok(k) = OldPublicKey::from_bech32(s.to_string()) {
            Ok(k)
        } else if let Ok(bytes) = hex_str_to_bytes(s) {
            if bytes.len() == 33 {
                let mut inner = [0; 33];
                inner.copy_from_slice(&bytes[0..33]);
                OldPublicKey::from_bytes(inner, OldPublicKey::DEFAULT_PREFIX)
            } else {
            ...
            match base64::decode(s) {
                Ok(bytes) => {
                    if bytes.len() == 33 {
                        let mut inner = [0; 33];
                        inner.copy_from_slice(&bytes[0..33]);
                        Ok(OldPublicKey::from_bytes(inner, OldPublicKey::DEFAULT_PREFIX)?)
                    } else {
...
impl fmt::Debug for OldPublicKey {
...
fn check_bech32() {
    ...
    let public_key = OldPublicKey::from_slice(&raw_bytes, OldPublicKey::DEFAULT_PREFIX)
    ...
    let check: Result<OldPublicKey, PublicKeyError> =
...
fn parse_base64_pubkey() {
    ...
    let _key: OldPublicKey = key.parse().unwrap();
}

#[test]
fn test_default_prefix() {
    OldPublicKey::from_bytes([0; 33], OldPublicKey::DEFAULT_PREFIX).unwrap();
}
