...
use crate::coin::Fee;
use crate::{address::Address, old_private_key::OldMessageArgs};
...
impl Contact {
    ...
    pub async fn old_get_chain_status(&self) -> Result<ChainStatus, CosmosGrpcError> {
    ...
    pub async fn old_get_latest_block(&self) -> Result<LatestBlock, CosmosGrpcError> {
        let mut grpc = TendermintServiceClient::connect(self.url.clone())
            .await?
            .accept_gzip();
        let syncing = grpc
    ...
    pub async fn old_get_block(&self, block: u64) -> Result<Option<Block>, CosmosGrpcError> {
        let mut grpc = TendermintServiceClient::connect(self.url.clone())
            .await?
            .accept_gzip();
    ...
    pub async fn old_get_block_range(
        &self,
        start: u64,
        end: u64,
    ...
    pub async fn old_get_block_params(&self) -> Result<BlockParams, CosmosGrpcError> {
        let res = self.old_get_param("baseapp", "BlockParams").await?;
    ...
    pub async fn old_get_account_info(&self, address: Address) -> Result<BaseAccount, CosmosGrpcError> {
    ...
    pub async fn old_get_tx_by_hash(&self, txhash: String) -> Result<GetTxResponse, CosmosGrpcError> {
    ...
    /// Grabs an up to date OldMessageArgs structure for an address,
    ...
    pub async fn old_get_message_args(
        &self,
        our_address: Address,
        fee: Fee,
    ) -> Result<OldMessageArgs, CosmosGrpcError> {
        let account_info = self.old_get_account_info(our_address).await?;

        let latest_block = self.old_get_latest_block().await?;
    ...
    pub async fn old_wait_for_next_block(&self, timeout: Duration) -> Result<(), CosmosGrpcError> {
        let start = Instant::now();
        let mut last_height = None;
        while Instant::now() - start < timeout {
            match (self.old_get_chain_status().await, last_height) {
